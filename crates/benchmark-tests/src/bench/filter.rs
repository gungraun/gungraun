...
pub(super) static NUMBERS_RE: LazyLock<Regex> = LazyLock::new(|| {
    Regex::new(
        r"(?x)
...
pub(super) static NUMBERS_DIFF_RE: LazyLock<Regex> =
    LazyLock::new(|| Regex::new(r"\([^)*]*\)(?:\s+\[[^\]]+\])?$").expect("Regex should compile"));

pub(super) static UNIT_RE: LazyLock<Regex> = LazyLock::new(|| {
    Regex::new(r"(?<prefix>)(?<unit>\s*\[[^\]]+\])(?<suffix>:\s*)").expect("Regex should compile")
});

pub(super) static RUNNING_RE: LazyLock<Regex> =
    LazyLock::new(|| Regex::new("^[ ]+Running .*$").expect("Regex should compile"));

pub(super) static PROCESS_DID_NOT_EXIT_SUCCESSFULLY_RE: LazyLock<Regex> = LazyLock::new(|| {
...
pub(super) static REGRESSION_SOFT_RE: LazyLock<Regex> = LazyLock::new(|| {
    Regex::new(
        r"(?x)
...
pub(super) static REGRESSION_HARD_RE: LazyLock<Regex> = LazyLock::new(|| {
    Regex::new(
        r"(?x)
...
pub(super) static SUMMARY_REGRESSION_SOFT_RE: LazyLock<Regex> = LazyLock::new(|| {
    Regex::new(
        r"(?x)
            ^(\s*[^0-9]+\()
            ([0-9.]+(?:\s*\[\S+\])?)
...
pub(super) static SUMMARY_REGRESSION_HARD_RE: LazyLock<Regex> = LazyLock::new(|| {
    Regex::new(
        r"(?x)
            ^(\s*[^0-9]+\()
            ([0-9.]+(?:\s*\[[^)]+\])?)
...
pub(super) static ABSOLUTE_PATH_APOSTROPHE_RE: LazyLock<Regex> =
    LazyLock::new(|| Regex::new("[']([/][^/']+)+[']").expect("Regex should compile"));

#[expect(clippy::multiple_inherent_impl)]
impl CapturedOutput {
    pub(super) fn normalize_coverage_stdout(stdout: &str) -> String {
    ...
    pub(super) fn filter_stderr(stderr: &[u8]) -> String {
        let mut result = String::new();
        let mut start = false;
        let mut first = false;
    ...
    pub(super) fn filter_stdout(&self, stdout: &[u8]) -> String {
        let mut result = String::new();
...
pub(super) fn filter_unit(desc: &str) -> Cow<'_, str> {
